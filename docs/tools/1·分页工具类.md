...
public class Service {
	...
	public PageUtils<DTO> queryList(Query query){
        // 数据总数
        int totalNumber = mapper.queryListCount(query);
        // 根据当前页码,页码大小,数据总数,构建PageUtils对象
        PageUtils<DTO> pageUtils = new PageUtils<>(query.getCurrentPage(), query.getPageSize(), totalNumber);
        query.setStartNum(pageUtils.getStartNum());
        if (queryNum > 0) {
            // 当前页数据
            pageUtils.setList(mapper.queryList(query));
        }
        return pageUtils;
	}
}
